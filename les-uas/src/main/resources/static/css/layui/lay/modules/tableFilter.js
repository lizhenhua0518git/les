...
layui.define(['table', 'jquery', 'form'], function (exports) {
	...
	tableFilter.cache = {}
	//筛选后的数据结构
	tableFilter.cacheData = new Array();
	tableFilter.eid = null;
	//渲染
		...
		var main = function (){
			...
			layui.each(filters, function(i, filter){
				...
				filterIcon.on("click", function(e) {
					...
					var l = $(this).offset().left - ($('body').outerWidth(true) - $(parent).outerWidth(true)) - 80 +"px";
					...
					form.on('submit(tableFilter)', function(data){
						...
						if(mode == "local"){
							tableFilter.cacheData = new Array();
							//本地交叉过滤
							var checkObject = $(parent).find("[type=checkbox]");
							checkObject.each(function(i, obj){
								$(checkObject[i]).removeAttr('checked');
							...
							if(trsIndex.length > 0 || data.field[filterName].length > 0){
								...
								for(var i=0;i<trsIndex.length;i++){
									tableFilter.cacheData[i]= table.cache[tableFilter.eid][trsIndex[i]];
								}
							}else{
								elem.next().find('.layui-table-body tr').removeClass("layui-hide")
								tableFilter.cacheData =table.cache[tableFilter.eid]; 
							}
						}else if(mode == "api"){
							//服务端交叉过滤
							//将数组转字符串
