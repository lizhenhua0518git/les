...
;!function (factory) {
  ...
  function getPosition(elem, _window, offsetTop) {
    ...
    if (offsetTop && _window.top !== _window.self) {
      ...
      offsetTemp.top += parseFloat(frameElem.css('borderTopWidth'));
      offsetTemp.left += parseFloat(frameElem.css('borderLeftWidth'));
    }
    ...
    // }
    var borderTopWidth = parseFloat($('html').css('borderTopWidth'));
    var borderLeftWidth = parseFloat($('html').css('borderLeftWidth'));
    return {
      top: (offsetTemp.top || 0) + elem.offset().top - bodyOffset.top - $(_window.document).scrollTop() + (isFirefox ?  0: borderTopWidth),
      left: (offsetTemp.left || 0) + elem.offset().left - bodyOffset.left - $(_window.document).scrollLeft() + (isFirefox ? 0 : borderLeftWidth)
    }
  }
