...
layui.define(['table'], function (exports) {
  ...
  table.Class.prototype.reverse = function () {
    ...
    that.layMain.css({marginLeft: isVertical ? that.layHeader.width() + 'px' : 0});
  ...
  $(document).on('mouseenter', '.' + ELEM_VIEW + ' ' + ELEM_FIXR + ' ' + ELEM_BODY, function (event) {
    ...
    // 处理鼠标移出左侧
    $(this).css({width: 'auto'}).closest(ELEM_FIXL).css({width: 'auto'});
  });
  ...
  var renderData = function () {
    ...
    that.renderData(resTemp, that.page, that.count, true);
    // 滚动到之前的位置
    that.layBody.scrollTop(scrollTop);
    that.layMain.scrollLeft(scrollLeft);
    // 执行done回调保证跟初始化一样
    typeof options.done === 'function' && options.done(resTemp, that.page, that.count);
