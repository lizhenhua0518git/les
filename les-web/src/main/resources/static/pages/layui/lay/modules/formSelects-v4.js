...
(function(layui, window, factory) {
	...
	Common.prototype.appender = function(){//针对IE做的一些拓展
		//拓展Array map方法
		if(!Array.prototype.map){Array.prototype.map=function(i,h){let b,a,c,e=Object(this),f=e.length>>>0;if(h){b=h}a=new Array(f);c=0;while(c<f){let d,g;if(c in e){d=e[c];g=i.call(b,d,c,e);a[c]=g}c++}return a}};

		//拓展Array foreach方法
		if(!Array.prototype.forEach){Array.prototype.forEach=function forEach(g,b){let d,c;if(this==null){throw new TypeError("this is null or not defined")}let f=Object(this);let a=f.length>>>0;if(typeof g!=="function"){throw new TypeError(g+" is not a function")}if(arguments.length>1){d=b}c=0;while(c<a){let e;if(c in f){e=f[c];g.call(d,e,c,f)}c++}}};

		//拓展Array filter方法
		if(!Array.prototype.filter){Array.prototype.filter=function(b){if(this===void 0||this===null){throw new TypeError()}let f=Object(this);let a=f.length>>>0;if(typeof b!=="function"){throw new TypeError()}let e=[];let d=arguments[1];for(let c=0;c<a;c++){if(c in f){let g=f[c];if(b.call(d,g,c,f)){e.push(g)}}}return e}};
	}
	...
	Common.prototype.init = function(target){
		...
		$((target ? target : `select[${NAME}]`)).each((index, select) => {
			...
			let $parent = $(`<div class="${PNAME}" FS_ID="${id}"></div>`);
	...
	Select4.prototype.value = function(id, type, isAppend){
		...
		if(typeof type == 'string' || type == undefined){
			let arr = [];
			if (data[id].values != undefined){
				arr = data[id].values.concat([]);
			}
			if(type == 'val') {
				return arr.map((val) => {
					return val.value;
		...
		if(common.isArray(type)) {
			...
			if(isAdd){
				data[id].values.forEach((val, index) => {
					temp[val.value] = 1;
				});
			}
			type.forEach((val, index) => {
				if(val && (!temp[val] || data[id].config.repeat)){
