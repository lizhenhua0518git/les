...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
    ...
    Class.prototype.config = {
        ...
        ,cellMinWidth: 40 //所有单元格默认最小宽度
        ,text: {
            none: '无数据'
        }
    };
    //默认树形结构展开
    Class.prototype.isOpen = true;
    //表格渲染
    Class.prototype.render = function(){
        ...
        //console.log("options",options,typeof options.isOpen);
                ...
                layui.each(data, function(i1, item1){
                    ...
                    that.eachCols(function(i3, item3){
                        ...
                        if(treeShowName==item3.field){//当前是否用于显示的值
                            ...
                            if(!o.isOpen)
                            	temTreeHtml='<i class="layui-icon layui-tree-head">&#xe623;</i> ';
                            var nbspHtml="<i>"//一次位移
                    ...
                    if(item1["level"]==1)
                    	...
                    	if(item1["isOpen"])
                    		...
                    		trs.push('<tr data-index="'+ i1 +'" style="display:none" upids="'+item1["upIds"]+'">'+ tds.join('') + '</tr>');
                    }
    ...
    Class.prototype.events = function(){
        ...
        /**
         * 递归将子节点下所有的isopen设置为false
         */
        function setIsOpen(data){
        	if(data && data.children.length>0){
        		var childData = data.children;
            	for(var i=0;i<childData.length;i++){
            		childData[i].isOpen =false;
            		setIsOpen(childData[i]);
            	}
            }
        }
        /**
        ...
        that.elem.on('click', 'i.layui-tree-head', function(){
            ...
            if(o.isOpen){//打开状态的，关闭
                sonO.hide();
                for(var i=0;i<sonO.length;i++){
                	var htmlValue = $(sonO[i]).find('.layui-tree-head').html();
                	if(htmlValue!="&nbsp;&nbsp;")
                		$(sonO[i]).find('.layui-tree-head').html('&#xe623;');
                }
                setIsOpen(o);
            }else{
            	var tempUpids;
            	var curUpid = "_"+o[options.treeId]+"_";
            	for(var i=0;i<sonO.length;i++){
            		tempUpids = $(sonO[i]).attr("upids");
            		if(tempUpids.indexOf(curUpid, tempUpids.length - curUpid.length) !== -1 ){
            			$(sonO[i]).show();
            		}
            	}
            }
