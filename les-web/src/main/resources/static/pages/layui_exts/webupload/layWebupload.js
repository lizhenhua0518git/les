layui.extend({
    ...
    //加载样式
    layui.link('/iles/pages/css/webUploader/webuploader.css');
    ...
    Class.prototype.init=function(){
        ...
        var fileBox=`<blockquote class="layui-elem-quote quoteBox">
                    ...
                    <button type="button" id="extent-button-uploader" class="layui-btn" style="height: 42px;margin-top: 5px;margin-left: 5px;font-size: 12px;">开始上传</button>
    ...
    Class.prototype.register=function () {
        ...
        var submitBtId = options.submitBtnId;
        var labelId = options.labelId;//labelId
        ...
        webUploader.Uploader.register({
            ...
            beforeSendFile:function(file){//利用md5File（）方法计算文件的唯一标记符
                ...
                (new webUploader.Uploader()).md5File(file,0,100*1024*1024).progress(function(percentage){
                }).then(function(val){
                    md5=this.md5File=file.fileMd5=val;
                    window.localStorage.md5 = val;
                    ...
                    $.ajax({
                            ...
                            success:function(response){
                                if(response.data ==""){
                                    ...
                                    fList.push(response.data);
                                    //存md5
                                    window.localStorage.md5 = response.data;
                                    $("#"+submitBtId).removeClass("layui-btn-disabled");
                                    $('#'+submitBtId).attr("disabled",false);
                                    $('#'+labelId).html(file.name+"上传成功");
                                    deferred.reject();
                                }
                            }
                        }
            ...
            beforeSend:function(block){//向后台发送当前文件的唯一标记，用于后台创建保存分块文件的目录
                ...
                (new webUploader.Uploader()).md5File(block.file,block.start,block.end).progress(function(percentage){
                    ...
                    $.ajax({
                            ...
                            data:{
                                // 文件唯一表示
                                fileMd5:window.localStorage.md5,
                                // 当前分块下标
                                chunk:block.chunk,
                                // 当前分块大小
                ...
                uplaod.options.formData.fileMd5 = window.localStorage.md5;
                uplaod.options.formData.chunk = block.chunk;
                return deferred.promise();
            },
                ...
                $.ajax({
                    ...
                    success:function(resdata){
                        if(resdata.data){//存在了，+1
                            ...
                            //修改这个全局的md5值，这里应该是个回调去获取当前文件的md5
                            $("#"+submitBtId).removeClass("layui-btn-disabled");
                            $('#'+submitBtId).attr("disabled",false);
                            $('#'+labelId).html(file.name+"上传成功");
                            that.clearData();
