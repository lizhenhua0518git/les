...
function getElement(item) {
    var tr = $('<tr>');
    var supplierName = subCHString(item.supplierName, 18);
    if(item.isUrgent==1){
        tr.append($("<td>").css('width','10%').css('color','#e60012').text(item.manualNo));
        tr.append($("<td>").css('width','25%').text(supplierName));
        tr.append($("<td>").css('width','24%').text(item.arrivalCode));
        tr.append($("<td>").css('width','14%').text(item.status));
        tr.append($("<td>").css('width','13%').text(item.queueTime));
        tr.append($("<td>").css('width','14%').text(item.waitTime));
	 
	}else{
		tr.append($("<td>").css('width','10%').text(item.manualNo));
	    tr.append($("<td>").css('width','25%').text(supplierName));
	    tr.append($("<td>").css('width','24%').text(item.arrivalCode));
	    tr.append($("<td>").css('width','14%').text(item.status));
	    tr.append($("<td>").css('width','13%').text(item.queueTime));
