...
function getElement(item, i) {
    var tr = $('<tr>');
    if(item.isUrgent==1){
     	    tr.append($("<td>").css('width','100px').css('color','#e60012').text((i > 8?(i+1):"0"+(i+1))));
    	   
 

    }else{
    	tr.append($("<td>").css('width','100px').text((i > 8?(i+1):"0"+(i+1))));
 	   
    }
    tr.append($("<td>").css('width','150px').text(item.status));
    tr.append($("<td>").css('width','130px').text(item.checkName));
    tr.append($("<td>").css('width','350px').text(item.arrivalCode));
    tr.append($("<td>").css('width','200px').text(item.teamName));
     if(item.waitTime==null){
    	 tr.append($("<td>").css('width','200px').text("未开始"));
     }else if(item.waitTime <0){
    	 tr.append($("<td>").css('width','200px').text(" "));
     }else{
