package com.zkzn.les.panel.util;
...
public class DateUtil {
    ...
    private static final int MILLI_SECOND = 1000; //毫秒进制
    private static final int SECOND_MINUTE=60; //分、秒进制
    private static final int HOUR_DAY= 24; //小时进制
    private DateUtil() {
	...
	public static Date formatStrToDate(String dateStr, String format){
		...
		Date date = null;
		if(StringUtil.isBlankOrEmpty(dateStr)){
			return date;
		}
		try {
	...
	public static String getDistanceTime(String str1, String str2) {  
        ...
        try {  
            ...
            day = diff / (HOUR_DAY * SECOND_MINUTE * SECOND_MINUTE * MILLI_SECOND);  
            hour = (diff / (SECOND_MINUTE * SECOND_MINUTE * MILLI_SECOND) - day * HOUR_DAY);  
            min = ((diff / (SECOND_MINUTE * MILLI_SECOND)) - day * HOUR_DAY * SECOND_MINUTE - HOUR_DAY * SECOND_MINUTE);  
            sec = (diff/MILLI_SECOND-day*HOUR_DAY*SECOND_MINUTE*SECOND_MINUTE-HOUR_DAY*SECOND_MINUTE*SECOND_MINUTE-min*SECOND_MINUTE);  
        } catch (ParseException e) {  
            e.printStackTrace();  
        }  
        String str = "";
	 ...
	 * @param str1 时间参数 1 格式：1990-01-01 12:00:00
	 * @param str2 时间参数 2 格式：2009-01-01 12:00:00
	 * @return 返回值为： xx小时xx分xx秒
	 */
	public static String getHourMin(String str1, String str2) {
		DateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

		String str = "";
		int value=dateTimeMinus(str1,str2,3);
	...
	public  static int dateTimeMinus(String fromDate, String toDate,int type){
        ...
        int days = (int) ((to - from)/(MILLI_SECOND * SECOND_MINUTE * SECOND_MINUTE * HOUR_DAY));
        int hours = (int) ((to - from)/(MILLI_SECOND * SECOND_MINUTE * SECOND_MINUTE));
		int minutes = (int) ((to - from)/(MILLI_SECOND * SECOND_MINUTE));
		if( type == 1){
			return days;	
		}else if(type == 2){
			return hours;	
	...
	public static String parseTimeStrAndAddSec(String timeStr,int sec){
		...
		try {
			 long time=sdf.parse(timeStr).getTime()+sec*MILLI_SECOND;
			 Date date=new Date();
			 date.setTime(time);
	...
	public static long getDistanceHour(Date dateOne,Date dateTwo){
         ...
         long day = diff / (HOUR_DAY * SECOND_MINUTE * SECOND_MINUTE * MILLI_SECOND); 
         long hour = (diff / (SECOND_MINUTE * SECOND_MINUTE * MILLI_SECOND) - day * HOUR_DAY);  
	...
	public static void main(String[] args) { 
   	String str;
	try {
		//str = getThisYear(getCurrentDateWithFormat("yyyy-MM-dd"));
		 System.out.println(dateTimeMinus("2020-08-25 15:00:00","2020-08-25 15:20:30",3));
	} catch (Exception e) {
		// TODO Auto-generated catch block
		e.printStackTrace();
	}
